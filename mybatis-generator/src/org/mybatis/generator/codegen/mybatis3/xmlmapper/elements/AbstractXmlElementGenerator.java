...
public abstract class AbstractXmlElementGenerator extends AbstractGenerator {
    ...
    protected XmlElement getXuidIncludeElement() {
        XmlElement ifElement = new XmlElement("if"); //$NON-NLS-1$
        ifElement.addAttribute(new Attribute("test", "xuid != null")); //$NON-NLS-1$ //$NON-NLS-2$

        TextElement includeElement = new TextElement("xuid = #{xuid,jdbcType=VARCHAR}"); //$NON-NLS-1$
