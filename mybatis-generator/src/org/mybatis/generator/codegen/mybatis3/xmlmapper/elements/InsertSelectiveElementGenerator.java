...
public class InsertSelectiveElementGenerator extends
    ...
    public void addElements(XmlElement parentElement) {
        ...
        for (IntrospectedColumn introspectedColumn : introspectedTable
            ...
            //mysql uuid
            if(context.getJdbcConnectionConfiguration().getDriverClass().indexOf("mysql")>0 && introspectedColumn.getActualColumnName().equalsIgnoreCase("xuid")){
//            	introspectedColumn.setDefaultValue("uuid()");
            	introspectedColumn.setUuid(true);
            	introspectedColumn.setUuidFunc("uuid()");
            }
            
//            <choose>
//            <when test="checkType = 'checkIfUiDataIdExistCount'">
//            uidata_id = #{uidataId}
//            </when>
//            <when test="checkType = 'checkIfTermAndMessageExistCount'">
//            and data like #{data}
//            </when>
//            <otherwise>
//            </otherwise>
//            </choose> 
            ...
            if(introspectedColumn.isUuid()){

            	 
            	 insertTrimElement.addElement(new TextElement(introspectedColumn.getActualColumnName() + ","));

            	 
            	 XmlElement valuesNotNullElement = new XmlElement("choose"); //$NON-NLS-1$
            	 XmlElement whenEle = new XmlElement("when");
            	 sb.setLength(0);
                 sb.append(introspectedColumn.getJavaProperty());
                 sb.append(" != null"); //$NON-NLS-1$
                 whenEle.addAttribute(new Attribute(
                 ...
                 whenEle.addElement(new TextElement(sb.toString()));
                 
            	 valuesNotNullElement.addElement(whenEle);
            	 
            	 XmlElement otherwiseEle = new XmlElement("otherwise");
            	 sb.setLength(0);
            	 sb.append(introspectedColumn.getUuidFunc()+",");
                 
              
                 
                 otherwiseEle.addElement(new TextElement(sb.toString()));
            	 valuesNotNullElement.addElement(otherwiseEle);
