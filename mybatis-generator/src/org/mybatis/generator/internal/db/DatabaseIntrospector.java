...
public class DatabaseIntrospector {
    ...
    private void calculateTableComment(FullyQualifiedTable table,
        ...
        try { 
            if (rs.next()) {
                String remark = rs.getString("REMARKS"); //$NON-NLS-1$
                if(StringUtils.isNullOrEmpty(remark) && databaseMetaData.getDatabaseProductName().toLowerCase().indexOf("mysql")>=0){
                	 String sql = "show table status " + (table
                             .getIntrospectedSchema() == null ? "" : " from " + table
                                     .getIntrospectedSchema() + " ") + " like '"
                             + table
                             .getIntrospectedTableName() + "' ";
                	 
                     PreparedStatement statement = databaseMetaData.getConnection().prepareStatement(sql);

                     ResultSet tableRs = statement.executeQuery();

                     if (tableRs.next()) {
                    	 remark = tableRs.getString("COMMENT");
                     }
                     tableRs.close();
                     statement.close();
                }
                System.out.println(table
                        .getIntrospectedTableName() + " remark : "+remark);
                if(remark!=null && remark.trim().length()>0){
                    introspectedTable.setRemark(remark);
                }
            }
    ...
    private Map<ActualTableName, List<IntrospectedColumn>> getColumns(
        ...
        while (rs.next()) {
            ...
            System.out.println("column: " + introspectedColumn.getActualColumnName() + " : " + remark);
    ...
    private List<IntrospectedTable> calculateIntrospectedTables(
        ...
        for (Map.Entry<ActualTableName, List<IntrospectedColumn>> entry : columns
            ...
            calculateTableComment(table, introspectedTable);
