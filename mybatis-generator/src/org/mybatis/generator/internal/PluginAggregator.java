...
public final class PluginAggregator implements Plugin {
    ...
    public boolean sqlMapDeleteByXuidElementGenerated(XmlElement element,
            IntrospectedTable table) {
        boolean rc = true;

        for (Plugin plugin : plugins) {
            if (!plugin
                    .sqlMapDeleteByXuidElementGenerated(element, table)) {
                rc = false;
                break;
            }
        }
    ...
    public boolean modelXuidClassGenerated(TopLevelClass tlc,
            IntrospectedTable introspectedTable) {
        boolean rc = true;

        for (Plugin plugin : plugins) {
            if (!plugin.modelXuidClassGenerated(tlc, introspectedTable)) {
                rc = false;
                break;
            }
        }
    ...
    public boolean sqlMapSelectScalarByExampleWithoutBLOBsElementGenerated(
            XmlElement element, IntrospectedTable introspectedTable) {
        boolean rc = true;

        for (Plugin plugin : plugins) {
            if (!plugin.sqlMapSelectScalarByExampleWithoutBLOBsElementGenerated(
                    element, introspectedTable)) {
                rc = false;
                break;
            }
    ...
    public boolean sqlMapSelectScalarByExampleWithBLOBsElementGenerated(
            XmlElement element, IntrospectedTable introspectedTable) {
        boolean rc = true;

        for (Plugin plugin : plugins) {
            if (!plugin.sqlMapSelectScalarByExampleWithBLOBsElementGenerated(element,
                    introspectedTable)) {
                rc = false;
                break;
            }
    ...
    public boolean sqlMapSelectByXuidElementGenerated(XmlElement element,
            IntrospectedTable introspectedTable) {
        boolean rc = true;

        for (Plugin plugin : plugins) {
            if (!plugin.sqlMapSelectByXuidElementGenerated(element,
                    introspectedTable)) {
                rc = false;
                break;
            }
    ...
    public boolean sqlMapUpdateByXuidSelectiveElementGenerated(
            XmlElement element, IntrospectedTable introspectedTable) {
        boolean rc = true;

        for (Plugin plugin : plugins) {
            if (!plugin.sqlMapUpdateByXuidSelectiveElementGenerated(
                    element, introspectedTable)) {
                rc = false;
                break;
            }
    ...
    public boolean sqlMapUpdateByXuidWithBLOBsElementGenerated(
            XmlElement element, IntrospectedTable introspectedTable) {
        boolean rc = true;

        for (Plugin plugin : plugins) {
            if (!plugin.sqlMapUpdateByXuidWithBLOBsElementGenerated(
                    element, introspectedTable)) {
                rc = false;
                break;
            }
    ...
    public boolean sqlMapUpdateByXuidWithoutBLOBsElementGenerated(
            XmlElement element, IntrospectedTable introspectedTable) {
        boolean rc = true;

        for (Plugin plugin : plugins) {
            if (!plugin.sqlMapUpdateByXuidWithoutBLOBsElementGenerated(
                    element, introspectedTable)) {
                rc = false;
                break;
            }
    ...
    public boolean clientDeleteByXuidMethodGenerated(Method method,
            Interface interfaze, IntrospectedTable introspectedTable) {
        boolean rc = true;

        for (Plugin plugin : plugins) {
            if (!plugin.clientDeleteByXuidMethodGenerated(method, interfaze,
                    introspectedTable)) {
                rc = false;
                break;
            }
    ...
    public boolean clientDeleteByXuidMethodGenerated(Method method,
        ...
        for (Plugin plugin : plugins) {
            if (!plugin.clientDeleteByXuidMethodGenerated(method,
                    topLevelClass, introspectedTable)) {
                rc = false;
                break;
            }
    ...
    public boolean clientSelectScalarByExampleWithBLOBsMethodGenerated(Method method,
            Interface interfaze, IntrospectedTable introspectedTable) {
        boolean rc = true;

        for (Plugin plugin : plugins) {
            if (!plugin.clientSelectScalarByExampleWithBLOBsMethodGenerated(method,
                    interfaze, introspectedTable)) {
                rc = false;
                break;
            }
    ...
    public boolean clientSelectScalarByExampleWithoutBLOBsMethodGenerated(Method method,
            Interface interfaze, IntrospectedTable introspectedTable) {
        boolean rc = true;

        for (Plugin plugin : plugins) {
            if (!plugin.clientSelectScalarByExampleWithoutBLOBsMethodGenerated(method,
                    interfaze, introspectedTable)) {
                rc = false;
                break;
            }
    ...
    public boolean clientSelectByXuidMethodGenerated(Method method,
            Interface interfaze, IntrospectedTable introspectedTable) {
        boolean rc = true;

        for (Plugin plugin : plugins) {
            if (!plugin.clientSelectByXuidMethodGenerated(method, interfaze,
                    introspectedTable)) {
                rc = false;
                break;
            }
    ...
    public boolean clientSelectByXuidMethodGenerated(Method method,
        ...
        for (Plugin plugin : plugins) {
            if (!plugin.clientSelectByXuidMethodGenerated(method,
                    topLevelClass, introspectedTable)) {
                rc = false;
                break;
            }
    ...
    public boolean clientUpdateByXuidSelectiveMethodGenerated(Method method,
            Interface interfaze, IntrospectedTable introspectedTable) {
        boolean rc = true;

        for (Plugin plugin : plugins) {
            if (!plugin.clientUpdateByXuidSelectiveMethodGenerated(method,
                    interfaze, introspectedTable)) {
                rc = false;
                break;
            }
    ...
    public boolean clientUpdateByXuidWithBLOBsMethodGenerated(Method method,
            Interface interfaze, IntrospectedTable introspectedTable) {
        boolean rc = true;

        for (Plugin plugin : plugins) {
            if (!plugin.clientUpdateByXuidWithBLOBsMethodGenerated(method,
                    interfaze, introspectedTable)) {
                rc = false;
                break;
            }
    ...
    public boolean clientUpdateByXuidWithoutBLOBsMethodGenerated(
            Method method, Interface interfaze,
            IntrospectedTable introspectedTable) {
        boolean rc = true;

        for (Plugin plugin : plugins) {
            if (!plugin.clientUpdateByXuidWithoutBLOBsMethodGenerated(
                    method, interfaze, introspectedTable)) {
                rc = false;
                break;
            }
    ...
    public boolean providerSelectScalarByExampleWithBLOBsMethodGenerated(
            Method method, TopLevelClass topLevelClass,
            IntrospectedTable introspectedTable) {
        boolean rc = true;

        for (Plugin plugin : plugins) {
            if (!plugin.providerSelectScalarByExampleWithBLOBsMethodGenerated(method,
                    topLevelClass, introspectedTable)) {
                rc = false;
                break;
            }
    ...
    public boolean providerSelectScalarByExampleWithoutBLOBsMethodGenerated(
            Method method, TopLevelClass topLevelClass,
            IntrospectedTable introspectedTable) {
        boolean rc = true;

        for (Plugin plugin : plugins) {
            if (!plugin.providerSelectScalarByExampleWithoutBLOBsMethodGenerated(method,
                    topLevelClass, introspectedTable)) {
                rc = false;
                break;
            }
    ...
    public boolean providerUpdateByXuidSelectiveMethodGenerated(
            Method method, TopLevelClass topLevelClass,
            IntrospectedTable introspectedTable) {
        boolean rc = true;

        for (Plugin plugin : plugins) {
            if (!plugin.providerUpdateByXuidSelectiveMethodGenerated(method,
                    topLevelClass, introspectedTable)) {
                rc = false;
                break;
            }
